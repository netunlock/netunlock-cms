...
# La consola de Windows no usa UTF-8 por defecto, y cuando la salida va a un
# archivo o a una tubería la codificación es la del sistema (cp1252 en español).
# Un mensaje con una flecha o un guion largo levantaba UnicodeEncodeError y
# volteaba la compilación entera. Se pide UTF-8 y, si no se puede, que los
# caracteres raros salgan reemplazados en vez de romper.
...
def _siembra_completa(base: Path) -> bool:
    """¿La base ya tiene el catálogo de fábrica adentro?

    El archivo existe desde que sqlite abre la conexión, así que su presencia no
    dice nada. Tampoco alcanza con mirar líneas, tipologías ni las fórmulas de
    vidrio: todo eso lo escribe ``seed_data.sembrar()``, y después sigue
    ``catalogo_mdt.sembrar_mdt()`` con los dos catálogos del fabricante, que es
    lo que más tarda. Lo último en entrar son los kits de MDT, así que un kit de
    una tipología MDT es la señal de que terminó de verdad.
    """
    import sqlite3
    try:
    ...
    try:
        return bool(cx.execute(
            "SELECT 1 FROM parametros WHERE clave = 'catalogo_mdt_version'"
        ).fetchone())
    except sqlite3.Error:
        return False
    finally:
...
def probar(version: str) -> None:
    ...
    sembrada = False
    # 60 s: la primera vez hay que sembrar los dos catálogos MDT enteros.
    for _ in range(60):
        time.sleep(1)
        ...
        # Se espera a que la siembra TERMINE. Cortar antes dejaba una base a
        # medio cargar, y como la siembra sólo corre cuando la base es nueva,
        # esa base quedaba incompleta para siempre.
        if base.exists() and _siembra_completa(base):
            sembrada = True
            break
    ...
    if not sembrada:
        raise SystemExit(
            "\n  FALLA  la base se creó pero el catálogo de fábrica no terminó "
            "de cargarse\n         en 60 segundos.")

    _log(f"  OK   arrancó sin pedir activación")
    _log(f"  OK   base creada y sembrada en {base.relative_to(SALIDA)}")

    # La prueba no puede dejar rastro: la carpeta tiene que quedar idéntica a lo
    # que hay adentro del zip. Si no, quien copia la carpeta al pendrive se
    # lleva la base de la prueba en vez de arrancar limpio.
    for residuo in ("datos", "salidas"):
        shutil.rmtree(carpeta / residuo, ignore_errors=True)
    _log("  OK   se borró la base de la prueba: la carpeta arranca limpia")
