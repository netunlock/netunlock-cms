...
def _tabla_barras(datos, unidades: int):
    # Las barras vienen optimizadas para toda la orden: multiplicarlas otra vez
    # por las unidades haría comprar el triple. Las órdenes emitidas antes de
    # este cambio no traen la marca y se siguen leyendo como antes.
    totalizadas = datos.get("barras_totalizadas", False)
    filas = [[_p(h, "celda_b") for h in
              ("Perfil", "Largo de barra", "Barras a preparar")]]
    for barra in datos.get("barras", []):
        cantidad = barra["barras"] if totalizadas else barra["barras"] * unidades
        filas.append([
