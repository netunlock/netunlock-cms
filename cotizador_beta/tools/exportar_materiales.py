...
from core.calculo import (calcular_presupuesto, presupuesto_desde_db,
                          resumen_materiales)
from core.despiece import barras_de_presupuesto
from reports import marca_prueba
...
def _hoja_barras(wb, pres) -> None:
    ...
    fila = 2
    for b in barras_de_presupuesto(pres.resultados):
...
# API
# ---------------------------------------------------------------------------
# Sello de versión de prueba
# ---------------------------------------------------------------------------

def _sellar(wb) -> None:
    """Cruza la planilla con el aviso de prueba, mientras no haya licencia.

    Una planilla no se puede cruzar en diagonal como un PDF, así que el aviso va
    donde no se puede pasar por alto ni recortar sin que se note: **las dos
    primeras filas de cada hoja**, en rojo, y en el encabezado de impresión, que
    sale en todas las páginas si alguien la manda a papel.
    """
    aviso = marca_prueba.aviso_planilla()
    if not aviso:
        return

    from openpyxl.styles import Alignment, Font, PatternFill

    rojo = PatternFill("solid", fgColor="C41717")
    titulo = Font(bold=True, color="FFFFFF", size=13)
    detalle = Font(bold=True, color="C41717", size=9)

    for ws in wb.worksheets:
        # Tres filas arriba de todo: el aviso, el detalle con el correo y una en
        # blanco para que no se pegue a los datos.
        ws.insert_rows(1, amount=3)

        c = ws.cell(row=1, column=1, value=aviso[0])
        c.font = titulo
        c.alignment = Alignment(vertical="center")
        # El relleno se estira a lo ancho de lo que use la hoja, para que se lea
        # como una banda y no como una celda suelta.
        for col in range(1, max(ws.max_column, 6) + 1):
            ws.cell(row=1, column=col).fill = rojo
        ws.row_dimensions[1].height = 22

        ws.cell(row=2, column=1, value=aviso[1]).font = detalle

        # Si la hoja tenía la fila de títulos congelada, ahora está tres más
        # abajo. Sin esto los encabezados se van al hacer scroll.
        if ws.freeze_panes == "A2":
            ws.freeze_panes = "A5"

        # Y en papel, arriba de cada página.
        ws.oddHeader.center.text = aviso[0]
        ws.oddHeader.center.color = "C41717"
        ws.oddHeader.center.size = 12
        ws.oddFooter.center.text = aviso[1]
        ws.oddFooter.center.color = "C41717"
