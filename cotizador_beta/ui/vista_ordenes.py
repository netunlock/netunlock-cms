...
class VistaOrdenes(ctk.CTkFrame):
    ...
    def _tabla(self):
        ...
        for texto, comando, ancho in (
                ...
                ("Diagrama de corte", self.diagrama_corte, 150),
    ...
    def diagrama_corte(self):
        """El plano de corte de esta orden, desde su despiece congelado."""
        fila = self._seleccion()
        if fila is None:
            return
        from reports.taller import diagramas_de_orden

        orden = ordenes.obtener(self.db, fila["numero"])
        carpeta = rutas.SALIDAS / "ordenes"
        try:
            destino = diagramas_de_orden(
                self.db, orden, carpeta / f"{orden['numero']}_corte.pdf")
        except ValueError as exc:
            messagebox.showwarning("Sin despiece", str(exc))
            return
        except Exception as exc:
            messagebox.showerror("No se pudo generar el diagrama", str(exc))
            return
        self.app.abrir_ruta(destino)
