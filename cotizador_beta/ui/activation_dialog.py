...
class VentanaActivacion(ctk.CTk):
    ...
    def _modo_licencia(self, cont):
        from core.prueba import CONTACTO, DIAS

        # Se distingue a quién se le está hablando. A alguien que acaba de
        # probar el programa tres días hay que decirle a dónde escribir; a un
        # cliente que ya lo usa y se le venció, que renueve. Es la misma
        # pantalla y no es el mismo mensaje.
        agotada = getattr(self.estado, "prueba_vencida", False)
        vencida = bool(self.estado.licencia)

        if agotada:
            titulo = "Se terminaron los días de prueba"
            bajada = (f"Usaste los {DIAS} días de prueba gratuita. Para seguir "
                      f"usándolo, escribime a {CONTACTO} con el código de este "
                      f"equipo y te mando la clave.")
        elif vencida:
            titulo = "Renovar suscripción"
            bajada = (f"Tu suscripción venció. Escribime a {CONTACTO} con el "
                      f"código de este equipo para renovarla.")
        else:
            titulo = "Activación de licencia"
            bajada = (f"Este equipo todavía no está registrado. Escribime a "
                      f"{CONTACTO} con el código de abajo y te mando la clave.")

        self._titulo(cont, titulo, bajada)
        self._aviso(cont, self.estado.motivo)

        # -- Correo, bien visible y para copiar de un clic
        fila_mail = ctk.CTkFrame(cont, fg_color="transparent")
        fila_mail.pack(fill="x", pady=(0, 14))
        ctk.CTkLabel(fila_mail, text=CONTACTO, font=tema.fuente(15, "bold"),
                     text_color=tema.c("acento"), anchor="w").pack(side="left")
        ctk.CTkButton(fila_mail, text="Copiar correo", width=120, height=30,
                      font=tema.fuente(11),
                      command=lambda: self._al_portapapeles(
                          CONTACTO, "Correo copiado.")).pack(side="right")

        # -- Código de equipo
        ctk.CTkLabel(cont, text="1.  Código de este equipo",
                     font=tema.fuente(13, "bold"), text_color=tema.c("texto"),
                     anchor="w").pack(anchor="w")
        ctk.CTkLabel(cont, text="Mandámelo por correo o WhatsApp.",
                     font=tema.fuente(11), text_color=tema.c("texto_suave"),
    ...
    def _al_portapapeles(self, texto: str, aviso: str) -> None:
        """Copia y avisa. Sirve para el código de equipo y para el correo."""
        try:
            self.clipboard_clear()
            self.clipboard_append(texto)
            self.update()
            messagebox.showinfo("Copiado", aviso, parent=self)
        except Exception:
            messagebox.showinfo("Copialo a mano", texto, parent=self)
