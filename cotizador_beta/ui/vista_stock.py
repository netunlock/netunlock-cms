...
class VistaStock(ctk.CTkFrame):
    ...
    def __init__(self, master, app):
        ...
        self.app = app
        self.db = app.db
        #: Con qué estado de 'usar_stock' se armó la pantalla. La aplicación
        #: cachea las vistas, así que sin esto prender el inventario no cambiaba
        #: nada hasta reiniciar el programa.
        self._armada_activa = None
        self._construir()

    def _construir(self):
        """Arma la pantalla según el inventario esté activo o no."""
        for hijo in self.winfo_children():
            hijo.destroy()
        for atributo in ("tabla", "tabla_mov", "combo_filtro", "buscar", "var_bajo"):
            if hasattr(self, atributo):
                delattr(self, atributo)

        self._armada_activa = stock.activo(self.db)
    ...
    def _panel_apagado(self):
        ...
        texto = (
            ...
            "Se activa con el botón de acá abajo, o poniendo el parámetro "
            "«usar_stock» en 1 desde Materiales y Costos → Costos operativos."
        )
    ...
    def _activar(self):
        ...
        self._construir()
        self.app.estado("Inventario activado. Usá «Alta de artículo» para empezar "
                        "a cargar saldos.")
    ...
    def al_entrar(self):
        """Se rearma sola si el inventario se prendió o se apagó desde Ajustes."""
        if stock.activo(self.db) != self._armada_activa:
            self._construir()
