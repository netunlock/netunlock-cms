...
from . import tema
from .dialogo_variables import BloqueVariables, resumen_opciones
...
class VistaCotizacion(ctk.CTkFrame):
    def __init__(self, master, app):
        ...
        #: Avisos del último recálculo, con el ítem que los originó
        self._avisos: list = []
    ...
    def _panel_items(self):
        ...
        # El contador va en la cabecera de la tarjeta y no en la fila de botones:
        # ahí abajo, con seis acciones, quedaba fuera de la pantalla — y un aviso
        # que no se ve no sirve para nada.
        self.zona_badge = ctk.CTkFrame(acciones, fg_color="transparent")
        self.zona_badge.pack(side="right", padx=(0, 12))
        self._badge = None
        ...
        for texto, comando, ancho in (("Editar", self.editar_item, 80),
                                      ...
                                      ("Barras a comprar", self.ver_barras, 140),
                                      ("Papeles de taller", self.papeles_taller, 145),
    ...
    def ver_barras(self):
        """Barras a comprar para todo el presupuesto, ya consolidadas."""
        from .ventana_barras import VentanaBarras

        if not self.items:
            messagebox.showwarning(
                "Sin aberturas",
                "Agregá al menos una abertura para ver las barras a comprar.")
            return
        VentanaBarras(self.winfo_toplevel(), self.app, self._armar())

    def papeles_taller(self):
        """Genera los tres papeles que baja el taller y abre la carpeta."""
        from reports import taller

        if not self.items:
            messagebox.showwarning("Sin aberturas",
                                   "Agregá al menos una abertura.")
            return

        presupuesto = self._armar()
        numero = (presupuesto.numero or "presupuesto").replace("/", "-")
        carpeta = rutas.SALIDAS / "taller" / numero
        try:
            generados = [
                taller.generar_diagramas(self.db, presupuesto, carpeta / "Corte.pdf"),
                taller.generar_listas(self.db, presupuesto, carpeta / "Materiales.pdf"),
                taller.generar_etiquetas(self.db, presupuesto, carpeta / "Etiquetas.pdf"),
            ]
        except PermissionError:
            messagebox.showerror(
                "Archivo en uso",
                "No se pudieron escribir los PDF.\n\nSi los tenés abiertos, "
                "cerralos y volvé a intentar.")
            return
        except Exception as exc:
            messagebox.showerror("Error al generar los papeles", str(exc))
            return

        self.app.estado(f"{len(generados)} papeles de taller generados.")
        if messagebox.askyesno(
                "Papeles de taller",
                "Se generaron:\n\n"
                "   Corte.pdf         diagramas de cada barra\n"
                "   Materiales.pdf    perfiles, vidrios y accesorios por rubro\n"
                "   Etiquetas.pdf     rótulos para pegar en las piezas\n\n"
                f"{carpeta}\n\n¿Abrir la carpeta?"):
    ...
    def recalcular(self, _evento=None):
        ...
        for resultado in self.presupuesto.resultados:
            item = resultado.item
            # El motor no sabe en qué renglón está: el origen se completa acá.
            for aviso in resultado.despiece.avisos:
                aviso.item = f"{item.orden} · {item.tipologia_nombre or item.tipologia_codigo}"
            ...
            # Las variables activas de la tipología también son opciones del ítem
            desde_variables = resumen_opciones(self.db, item)
            if desde_variables:
                opcionales.append(desde_variables)
        ...
        if avisos_mod.contar(self._avisos)[0]:
            resumen += f"   ·   ⚠ {avisos_mod.resumen(self._avisos)}"
        if self._muestra_consumo:
    ...
    def _pintar_badge(self):
        """Actualiza el contador de avisos con el color que corresponda.

        Reconfigura el botón existente en vez de rehacerlo: esto corre en cada
        recálculo —o sea, con cada tecla que se tipea en el margen— y destruir
        el botón cuya orden se está ejecutando puede dejar la interfaz trabada.
        """
        from .panel_avisos import badge

        nuevo = badge(self.zona_badge, self._avisos, self.ver_avisos, self._badge)
        if nuevo is not self._badge:
            self._badge = nuevo
            self._badge.pack()

    def ver_avisos(self):
        """Abre la lista accionable de problemas del presupuesto."""
        from .panel_avisos import VentanaAvisos

        if not self._avisos:
            return

        def recalcular_y_devolver():
            """Tras arreglar algo, se recalcula todo y se devuelve la lista nueva.

            Se recalcula el presupuesto entero y no sólo el ítem afectado: un
            precio que faltaba suele faltar en varias aberturas a la vez, y
            arreglarlo una vez tiene que apagar todos sus avisos.
            """
            self.recalcular()
            return self._avisos

        VentanaAvisos(self.winfo_toplevel(), self.app, self._avisos,
                      al_resolver=recalcular_y_devolver)
    ...
    def guardar(self, silencioso: bool = False) -> bool:
        ...
        items = [{
            ...
            "variables_json": variables_tipologia.escribir_json(r.item.variables),
    ...
    def cargar_presupuesto(self, pid: int):
        ...
        self.items = [Item(
            ...
            variables=variables_tipologia.leer_json(
                self._columna(f, "variables_json", "")),
        ) for f in filas]
...
class DialogoItem(DialogoBase):
    ...
    def _panel_datos(self, master):
        ...
        # Aviso de conjunto: las medidas que se cargan abajo son las del conjunto
        # entero, no las de cada paño.
        self.lbl_composicion = ctk.CTkLabel(
            cuerpo, text="", font=tema.fuente(11), text_color=tema.c("acento"),
            anchor="w", justify="left", wraplength=560)
        self.lbl_composicion.grid(row=1, column=0, columnspan=3, sticky="ew")
        self.lbl_composicion.grid_remove()

        self.combo_linea = CampoCombo(cuerpo, "Línea", list(self.lineas), ancho=180,
                                      al_cambiar=self._cambiar_linea)
        self.combo_linea.grid(row=2, column=0, sticky="ew", padx=(0, 10), pady=4)
        self.combo_color = CampoCombo(cuerpo, "Color / Terminación", (), ancho=180,
                                      al_cambiar=self._previsualizar)
        self.combo_color.grid(row=2, column=1, sticky="ew", padx=(0, 10), pady=4)
        self.combo_vidrio = CampoCombo(cuerpo, "Vidrio", list(self.vidrios), ancho=180,
                                       al_cambiar=self._previsualizar)
        self.combo_vidrio.grid(row=2, column=2, sticky="ew", pady=4)

        self.campo_ancho = CampoUnidad(cuerpo, "Ancho (A)", "milimetros", 1500, ancho=110,
                                       entero=True, al_cambiar=self._previsualizar)
        self.campo_ancho.grid(row=3, column=0, sticky="ew", padx=(0, 10), pady=4)
        self.campo_alto = CampoUnidad(cuerpo, "Alto (H)", "milimetros", 1100, ancho=110,
                                      entero=True, al_cambiar=self._previsualizar)
        self.campo_alto.grid(row=3, column=1, sticky="ew", padx=(0, 10), pady=4)

        cantidades = ctk.CTkFrame(cuerpo, fg_color="transparent")
        cantidades.grid(row=3, column=2, sticky="ew", pady=4)
        ...
        opciones.grid(row=4, column=0, columnspan=3, sticky="ew", pady=(12, 4))
        ...
        # Bloque de variables de la tipología. Se arma vacío y se rellena en
        # _refrescar_variables(), que corre cada vez que cambia la tipología o
        # la línea: el juego de opciones es otro.
        self.bloque_variables = BloqueVariables(cuerpo, self.db,
                                                al_cambiar=self._previsualizar)
        self.bloque_variables.grid(row=5, column=0, columnspan=3, sticky="ew",
                                   pady=(10, 0))
        self.bloque_variables.grid_remove()
        ...
        self.campo_obs.grid(row=6, column=0, columnspan=3, sticky="ew", pady=(6, 0))
    ...
    def _precargar(self, item):
        ...
        if item:
            ...
            # Si la tipología corta el premarco o el mosquitero con sus perfiles,
            # la casilla muestra lo que de verdad se corta: una abertura guardada
            # con la opción elegida aparte puede traerlo distinto.
            contexto = variables_tipologia.resolver(self.db, item)
            ...
            # Después de armar el bloque, se le ponen los valores del ítem
            self._refrescar_variables(item.variables)
        else:
            if claves:
    ...
    def _refrescar_variables(self, valores: dict | None = None):
        """Redibuja el bloque de opciones para la tipología y línea actuales."""
        tip = self.tipologias.get(self.combo_tipologia.get())
        linea = self.lineas.get(self.combo_linea.get())
        ...
        hay = self.bloque_variables.construir(tip["codigo"], linea["id"], valores)
        # Una tipología sin opciones no muestra una tarjeta vacía: ocuparía lugar
        # y haría pensar que falta completar algo.
        if hay:
            self.bloque_variables.grid()
        else:
            self.bloque_variables.grid_remove()

        # Si es un conjunto, decir de qué está hecho: quien carga la abertura
        # tiene que saber que ese ancho y ese alto son del conjunto entero.
        composicion = COMP.describir(self.db, tip["codigo"], linea["id"])
        if composicion:
            self.lbl_composicion.configure(
                text=f"Conjunto de {len(COMP.panos(self.db, tip['codigo'], linea['id']))} "
                     f"paños:  {composicion}")
            self.lbl_composicion.grid()
        else:
            self.lbl_composicion.grid_remove()
    ...
    def _construir_item(self) -> Item | None:
        ...
        return Item(
            ...
            variables=self.bloque_variables.valores(),
