...
class App(ctk.CTk):
    def __init__(self, ajustes: Ajustes):
        ...
        self.db = DB()
        # La fecha de inicio de la prueba se guarda por duplicado: en la carpeta
        # del usuario y adentro de la base. El control de licencia corre antes de
        # que la base exista, así que la copia de acá se repone recién ahora.
        from core import prueba as _prueba
        _prueba.sincronizar()
        ...
        # Un cuelgue de Tkinter no deja traceback: la ventana se congela y del
        # otro lado no queda nada para mirar. El vigilante anota la pila de todos
        # los hilos en datos/cuelgue.log cuando la interfaz deja de responder.
        from core.vigilante import vigilar

        self.vigilante = vigilar(self)
    ...
    def _mensaje_inicial(self):
        ...
        if getattr(estado, "es_prueba", False):
            # Durante la prueba el contador va siempre a la vista, en la barra
            # de abajo. Enterarse el cuarto día, cuando ya no abre, se siente
            # como que se cortó de golpe.
            from core.prueba import CONTACTO
            self.estado(f"{estado.motivo}   ·   Para seguir usándolo: {CONTACTO}",
                        f"v{rutas.version()}")
            # Y un aviso una sola vez el último día, que es cuando hay que
            # hacer algo.
            if estado.dias_restantes == 1 and self.ajustes.aviso_licencia_visto != "prueba-1":
                self.ajustes.aviso_licencia_visto = "prueba-1"
                self.ajustes.guardar()
                self.after(900, lambda: messagebox.showinfo(
                    "Último día de prueba",
                    f"Hoy es el último día de la prueba gratuita.\n\n"
                    f"Si te sirvió, escribime a {CONTACTO} con el código de "
                    f"equipo que aparece en Ajustes → Licencia y te mando la "
                    f"clave.\n\nLo que cargaste no se pierde: los presupuestos, "
                    f"los precios y las fórmulas quedan donde están."))

        elif estado.por_vencer and self.ajustes.aviso_licencia_visto != estado.licencia.vence:
            self.ajustes.aviso_licencia_visto = estado.licencia.vence
            self.ajustes.guardar()
            self.after(700, lambda: messagebox.showwarning(
                "Licencia por vencer",
                f"{estado.aviso}\n\nEscribime para renovarla y evitar "
                "interrupciones."))
