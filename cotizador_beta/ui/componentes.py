...
class Tabla(ctk.CTkFrame):
    ...
    def datos_de_filas(self) -> list[dict]:
        """Los datos de cada fila, en el orden en que se ven."""
        return [self._filas.get(iid, {}) for iid in self.tree.get_children()]
...
class DialogoBase(ctk.CTkToplevel):
    """Ventana centrada sobre el padre, con Escape para cerrar.

    ``modal=True`` (lo normal en un formulario) toma el foco con ``grab_set()``:
    mientras esté abierta, el resto de la aplicación no responde. Eso está bien
    para algo que **exige una respuesta** —cargar un precio, confirmar un
    borrado— y está mal para un informe.

    Una ventana de consulta va con ``modal=False``: se puede dejar abierta al
    costado y seguir trabajando. Si se la hace modal, y encima aparece detrás de
    la ventana principal, desde afuera se ve exactamente como un cuelgue.
    """
    ...
    def _presentar(self):
        ...
        try:
            ...
            pass
        self._asegurar_visible()

    #: Cada cuánto se controla que la ventana siga en pantalla, y por cuánto
    #: tiempo. 1,8 s cubre de sobra el ciclo de CustomTkinter, que en esta
    #: máquina esconde la ventana recién a los 767 ms.
    VIGILAR_CADA_MS = 120
    VIGILAR_TICKS = 15

    def _asegurar_visible(self, tick: int = 0):
        """Vigila que la ventana esté de verdad en pantalla durante el arranque.

        Al crear un ``Toplevel``, CustomTkinter lo esconde para pintarle la barra
        de título de Windows y lo vuelve a mostrar con un ``after()`` propio.
        Cada ``geometry()`` y cada ``resizable()`` dispara ese ciclo de nuevo, y
        alguno de esos ciclos deja la ventana **creada pero sin mapear**: existe,
        tiene geometría, y no se ve. Desde afuera parece que el botón no hace
        nada.

        Un diálogo modal lo disimulaba porque ``grab_set()`` la fuerza a
        aparecer; un informe no modal quedaba invisible.

        Por eso no alcanza con mostrarla una vez ni con dejar de mirar apenas se
        ve: medido en esta máquina, la ventana aparecía a los 294 ms y volvía a
        esconderse a los 767 ms. Se controla cada tanto durante el arranque y se
        la vuelve a mostrar las veces que haga falta. Después de eso ya nadie la
        toca y la vigilancia se apaga sola.
        """
        try:
            if not self.winfo_exists():
                return
            ...
            return
        if tick < self.VIGILAR_TICKS:
            self.after(self.VIGILAR_CADA_MS, self._asegurar_visible, tick + 1)
