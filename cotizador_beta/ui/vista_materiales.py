...
from core import composicion as COMP
...
class PanelABM(Tarjeta):
    ...
    def seleccionar_donde(self, columna: str, valor) -> bool:
        """Selecciona la primera fila cuyo campo coincida con ``valor``.

        La usa el panel de avisos para dejar marcado el registro con problemas
        al saltar desde la lista de errores.
        """
        if valor in (None, ""):
            return False
        for indice, datos in enumerate(self.tabla.datos_de_filas()):
            if str(datos.get(columna, "")) == str(valor):
...
class VistaMateriales(ctk.CTkFrame):
    ...
    def ir_a(self, objeto: dict) -> None:
        """Abre la pestaña donde se arregla un aviso, con los filtros puestos.

        La usa el panel de avisos: si el problema es que falta una fórmula, no
        alcanza con abrir Materiales — hay que dejar la tipología y la línea ya
        seleccionadas, que es el paso que el usuario hacía a mano.
        """
        tipo = (objeto or {}).get("tipo", "")
        linea = objeto.get("linea", "")

        if tipo in ("perfil", "perfil_nuevo"):
            self.pestanias.set("  Perfiles  ")
            if linea:
                self.combo_perfil_linea.set(linea)
                self.abm_perfiles.refrescar()
            self.abm_perfiles.seleccionar_donde("codigo", objeto.get("codigo"))

        elif tipo == "linea_precio":
            self.pestanias.set("  Líneas y precios  ")
            self.abm_lineas.seleccionar_donde("nombre", linea)
            self.abm_precios.refrescar()
            self.abm_precios.seleccionar_donde("color", objeto.get("color"))

        elif tipo == "formulas":
            self.pestanias.set("  Fórmulas de despiece  ")
            # La etiqueta del combo es "CODIGO — Nombre": se rearma desde la base
            # en vez de leer las opciones del control.
            tip = self.db.tipologia(objeto.get("tipologia", ""))
            if tip:
                self.combo_f_tip.set(f"{tip['codigo']} — {tip['nombre']}")
            self.combo_f_linea.set(linea or "(Genérica — todas las líneas)")
            self.abm_formulas.refrescar()
            self.abm_formulas.seleccionar_donde("perfil_codigo", objeto.get("perfil"))

        elif tipo == "composicion":
            self.pestanias.set("  Tipologías  ")
            self.abm_tipologias.seleccionar_donde("codigo", objeto.get("tipologia"))
            self._al_elegir_tipologia()

        elif tipo in ("kit", "kit_item"):
            self.pestanias.set("  Kits  ")
            self.abm_kits.refrescar()

        elif tipo == "accesorio":
            self.pestanias.set("  Accesorios  ")
    ...
    def _panel_kits(self, hoja):
        ...
        def campos_item(_fila):
            ...
            # Variables de tipo 'accesorio' del kit: son las únicas que pueden
            # decidir qué accesorio entra. Si el kit es de una tipología, se
            # ofrecen las de esa tipología; si es genérico, todas las que haya.
            kit = self.abm_kits.seleccionado()
            elegibles = self.db.query(
                "SELECT DISTINCT clave, etiqueta FROM tipologia_variables "
                "WHERE tipo = 'accesorio' AND activo = 1"
                + (" AND tipologia_codigo = ?" if kit and kit["tipologia_codigo"] else "")
                + " ORDER BY clave",
                (kit["tipologia_codigo"],) if kit and kit["tipologia_codigo"] else ())
            return [
                ...
                {"key": "variable_clave", "label": "Lo elige la opción", "tipo": "combo",
                 "opciones": [""] + [v["clave"] for v in elegibles],
                 "ayuda": ("Dejalo vacío para que el accesorio sea siempre el de "
                           "arriba. Si elegís una opción de la tipología, el de "
                           "arriba pasa a ser el valor por defecto y manda lo que "
                           "se elija al cargar la abertura."
                           + ("   ·   " + "  ·  ".join(
                               f"{v['clave']} = {v['etiqueta']}" for v in elegibles)
                              if elegibles else
                              "   ·   Esta tipología todavía no declara ninguna "
                              "opción de tipo accesorio."))},
            ]
        ...
        def guardar_item(id_, datos):
            ...
            registro = {"kit_id": kid, "accesorio_id": accesorio["id"],
                        ...
                        "variable_clave": (datos.get("variable_clave") or "").strip()}
            if id_:
        ...
        self.abm_kit_items = PanelABM(
            ...
            [("codigo", "Código", 120, "w"), ("descripcion", "Accesorio", 260, "w"),
             ("unidad", "Un.", 70, "center"),
             ("cantidad_formula", "Cantidad (fórmula)", 160, "w"),
             ("variable_clave", "Lo elige", 110, "w"),
             ("precio", "Precio", 130, "e")],
    ...
    def _panel_tipologias(self, hoja):
        hoja.columnconfigure(0, weight=3)
        hoja.columnconfigure(1, weight=2)
        # La columna derecha lleva dos paneles apilados: las opciones de la
        # tipología arriba y su composición abajo.
        hoja.rowconfigure(0, weight=1)
        hoja.rowconfigure(1, weight=1)
        ...
        def guardar(id_, datos):
            ...
            # Cambiar el CÓDIGO es cambiar la clave por la que las fórmulas, las
            # variables y los kits encuentran a su tipología. Sin arrastrarlos,
            # quedan huérfanos: siguen en la base pero la pantalla los muestra
            # vacíos, y parece que se borraron.
            anterior = self.db.query_one("SELECT * FROM tipologias WHERE id = ?", (id_,))
            codigo_viejo = anterior["codigo"] if anterior else ""
            codigo_nuevo = (datos.get("codigo") or "").strip()

            if codigo_viejo and codigo_nuevo and codigo_viejo != codigo_nuevo:
                colgantes = COMP.dependencias(self.db, codigo_viejo)
                if colgantes:
                    detalle = "\n".join(f"   {n}  {etiqueta}"
                                        for etiqueta, n in colgantes)
                    respuesta = messagebox.askyesnocancel(
                        "Cambiar el código de la tipología",
                        f"«{codigo_viejo}» pasa a llamarse «{codigo_nuevo}».\n\n"
                        f"Cuelgan de ese código:\n{detalle}\n\n"
                        "SÍ  ·  conservar todo con el código nuevo\n"
                        "NO  ·  empezar de cero, dejando lo anterior en «"
                        f"{codigo_viejo}»\n\n"
                        "¿Conservar lo cargado?")
                    if respuesta is None:
                        return
                    if respuesta:
                        COMP.renombrar_codigo(self.db, codigo_viejo, codigo_nuevo)

            self.db.actualizar("tipologias", id_, datos)
            self.app.recargar_catalogos()

        def duplicar():
            """Clona la tipología seleccionada con todo lo que cuelga de ella."""
            fila = self.abm_tipologias.seleccionado()
            if fila is None:
                messagebox.showinfo("Elegí una tipología",
                                    "Seleccioná la tipología que querés duplicar.")
                return

            resumen = COMP.resumen_de_tipologia(self.db, fila["codigo"])
            dialogo = DialogoFormulario(
                self.winfo_toplevel(), f"Duplicar «{fila['codigo']}»",
                [{"key": "codigo", "label": "Código de la tipología nueva",
                  "requerido": True, "default": f"{fila['codigo']}B",
                  "ayuda": f"Se copian: {resumen}."},
            ...
            try:
                nuevo = COMP.duplicar_tipologia(
                    self.db, fila["codigo"], dialogo.resultado["codigo"],
                    dialogo.resultado.get("nombre", ""))
            except ValueError as exc:
                messagebox.showerror("No se pudo duplicar", str(exc))
                return

            self.abm_tipologias.refrescar()
            self.abm_tipologias.seleccionar_donde("codigo", nuevo)
            self._al_elegir_tipologia()
            self.app.recargar_catalogos()
            messagebox.showinfo(
                "Tipología duplicada",
                f"Se creó «{nuevo}» con {COMP.resumen_de_tipologia(self.db, nuevo)}.\n\n"
                "Ahora podés cambiarle lo que necesites sin tocar la original.")

        self.abm_tipologias = PanelABM(
            hoja, "Tipologías de abertura",
            [("codigo", "Código", 90, "w"), ("nombre", "Nombre", 250, "w"),
             ("hojas_default", "Hojas", 70, "center"), ("esquema", "Esquema", 110, "w"),
             ("horas_por_m2", "Fabricación / m²", 130, "e"),
             ("admite_mosquitero", "Mosq.", 70, "center"),
             ("admite_premarco", "Prem.", 70, "center"),
             ("activo", "Activa", 65, "center")],
            ...
            al_elegir=self._al_elegir_tipologia,
            extras=(("Duplicar…", duplicar),),
        ).formatos({"horas_por_m2": "horas", "admite_mosquitero": "bool",
                    "admite_premarco": "bool", "activo": "bool"})
        self.abm_tipologias.grid(row=0, column=0, rowspan=2, sticky="nsew",
                                 padx=(0, 10), pady=6)

        self._panel_variables(hoja)
        self._panel_composicion(hoja)

    def _al_elegir_tipologia(self):
        self.abm_variables.refrescar()
        self.abm_composicion.refrescar()

    # =====================================================================
    # Variables de la tipología seleccionada
    # =====================================================================

    def _panel_variables(self, hoja):
        """Las opciones que la tipología ofrece al cargar una abertura.

        Cuelga de la tipología porque es donde vive el concepto: una variable no
        tiene sentido sola, sólo dentro de la tipología que la declara y de las
        fórmulas que la usan.
        """
        def tipologia_actual():
            fila = self.abm_tipologias.seleccionado()
            return fila["codigo"] if fila else ""

        def cargar():
            codigo = tipologia_actual()
            if not codigo:
                return []
            return self.db.query(
                "SELECT tv.*, l.nombre AS linea_txt FROM tipologia_variables tv "
                "LEFT JOIN lineas l ON l.id = tv.linea_id "
                "WHERE tv.tipologia_codigo = ? ORDER BY tv.orden, tv.id", (codigo,))

        def campos(_fila):
            return [
                {"key": "clave", "label": "Clave (así se escribe en la fórmula)",
                 "requerido": True, "placeholder": "PREMARCO · ALTO_FIJO",
                 "ayuda": "Se normaliza sola: 'alto fijo' queda ALTO_FIJO. "
                          "No puede llamarse A, H, N, AH, HH, PERIM ni M2."},
                {"key": "etiqueta", "label": "Cómo se lee en el formulario",
                 "requerido": True, "placeholder": "Lleva premarco"},
                {"key": "tipo", "label": "Tipo", "tipo": "combo",
                 "opciones": list(V.TIPOS), "default": "bool",
                 "ayuda": "bool = casilla (1/0) · numero = campo numérico · "
                          "opcion = lista desplegable · accesorio = elegir del "
                          "catálogo (rueda simple o doble, cierre de embutir o "
                          "multipunto)"},
                {"key": "opciones", "label": "Opciones",
                 "placeholder": '[["Cierre simple", 0], ["Multipunto", 1]]   ·   R48, R49',
                 "ayuda": "Para tipo 'opcion': cada opción es un par, lo que se "
                          "lee y el número que toma la variable en la fórmula.   "
                          "Para tipo 'accesorio': los códigos elegibles separados "
                          "por coma (R48, R49). Vacío = todo el catálogo."},
                {"key": "valor_default", "label": "Valor por defecto", "default": "0"},
                {"key": "minimo", "label": "Mínimo", "tipo": "decimal", "decimales": 1},
                {"key": "maximo", "label": "Máximo", "tipo": "decimal", "decimales": 1,
                 "ayuda": "0 = sin tope"},
                {"key": "linea_txt", "label": "Sólo para la línea", "tipo": "combo",
                 "opciones": [""] + [l["nombre"] for l in self.db.lineas()],
                 "ayuda": "Vacío = vale para la tipología en todas las líneas"},
                {"key": "ayuda", "label": "Ayuda para el usuario"},
        ...
        def guardar(id_, datos):
            ...
            if not codigo:
                messagebox.showinfo("Elegí una tipología",
                                    "Seleccioná primero la tipología de la izquierda.")
                return
            datos = dict(datos)
            datos["tipologia_codigo"] = codigo
            datos["linea_id"] = self._id_linea(datos.pop("linea_txt", "")) or None
            try:
                V.guardar(self.db, datos, id_)
            except V.ClaveEnUso as exc:
                # Renombrar a secas dejaría las fórmulas apuntando a un nombre
                # que ya no existe, y el error aparecería recién al cotizar.
                if not messagebox.askyesno(
                        "La variable está en uso",
                        f"«{exc.clave_vieja}» se usa en {len(exc.usos)} fórmula(s) "
                        f"de despiece.\n\n"
                        f"¿Renombrarla también ahí, de «{exc.clave_vieja}» a "
                        f"«{exc.clave_nueva}»?\n\n"
                        "Si decís que no, no se guarda nada."):
                    return
                try:
                    V.guardar(self.db, datos, id_, renombrar_formulas=True)
                except ValueError as otro:
                    messagebox.showerror("No se puede guardar la variable", str(otro))
                    return
                self.abm_formulas.refrescar()
            except ValueError as exc:
                messagebox.showerror("No se puede guardar la variable", str(exc))
                return
            self.app.recargar_catalogos()

        def borrar(id_):
            fila = self.db.query_one(
                "SELECT * FROM tipologia_variables WHERE id = ?", (id_,))
            if fila is None:
                return
            # Borrar una variable que una fórmula usa la deja valiendo cero y la
            # pieza desaparece del despiece sin que nadie se entere: hay que avisar.
            usos = V.en_uso(self.db, fila["tipologia_codigo"], fila["clave"])
            if usos and not messagebox.askyesno(
                    "La variable está en uso",
                    f"«{fila['clave']}» se usa en {len(usos)} fórmula(s) de despiece.\n\n"
                    "Si la borrás, esas fórmulas la van a evaluar como 0 y las piezas "
                    "condicionales van a desaparecer del despiece.\n\n¿Borrarla igual?"):
                return
            self.db.borrar("tipologia_variables", id_)
            self.app.recargar_catalogos()

        self.abm_variables = PanelABM(
            hoja, "Opciones de la tipología seleccionada",
            [("clave", "Clave", 130, "w"), ("etiqueta", "Etiqueta", 200, "w"),
             ("tipo", "Tipo", 80, "center"),
             ("valor_default", "Por defecto", 90, "center"),
             ("linea_txt", "Línea", 120, "w"),
             ("activo", "Activa", 65, "center")],
            cargar=cargar, campos=campos, al_guardar=guardar, al_borrar=borrar,
            alto=7,
        ).formatos({"activo": "bool"})
        self.abm_variables.grid(row=0, column=1, sticky="nsew", pady=(6, 5))

    # =====================================================================
    # Composición: de qué paños está hecha una tipología
    # =====================================================================

    def _panel_composicion(self, hoja):
        """Los paños que forman una tipología compuesta.

        Una corrediza con paño fijo abajo es una COR2 arriba y un FIJO abajo.
        Cada paño recibe su medida por fórmula, evaluada con las variables de la
        compuesta, así el reparto del hueco siempre cierra.
        """
        def tipologia_actual():
            fila = self.abm_tipologias.seleccionado()
            return fila["codigo"] if fila else ""
        ...
        def cargar():
            ...
            if not codigo:
                return []
            return self.db.query(
                "SELECT tc.*, l.nombre AS linea_txt FROM tipologia_composicion tc "
                "LEFT JOIN lineas l ON l.id = tc.linea_id "
                "WHERE tc.tipologia_codigo = ? ORDER BY tc.orden, tc.id", (codigo,))

        def campos(_fila):
            codigo = tipologia_actual()
            candidatas = [t["codigo"] for t in self.db.tipologias()
                          if t["codigo"] != codigo]
            return [
                {"key": "orden", "label": "Orden", "tipo": "entero", "default": 1,
                 "ayuda": "De arriba hacia abajo, o de izquierda a derecha"},
                {"key": "tipologia_hijo", "label": "Tipología del paño",
                 "tipo": "combo", "opciones": candidatas, "requerido": True},
                {"key": "etiqueta", "label": "Cómo se llama este paño",
                 "placeholder": "Corrediza superior · Paño fijo inferior",
                 "ayuda": "Aparece en la lista de corte delante de cada pieza"},
                {"key": "formula_ancho", "label": "Ancho del paño", "default": "A",
                 "ayuda": AYUDA_VARIABLES},
                {"key": "formula_alto", "label": "Alto del paño", "default": "H",
                 "ayuda": "Ej.: H - ALTO_FIJO - 45  (los 45 se los lleva el acople)"},
                {"key": "formula_hojas", "label": "Hojas del paño", "default": "N"},
                {"key": "linea_txt", "label": "Sólo para la línea", "tipo": "combo",
                 "opciones": [""] + [l["nombre"] for l in self.db.lineas()],
                 "ayuda": "Vacío = vale para todas las líneas"},
                {"key": "activo", "label": "Activo", "tipo": "bool", "default": 1},
            ]
        ...
        def guardar(id_, datos):
            ...
            lid = datos["linea_id"]
            nombres = V.nombres(self.db, codigo, lid)
            for clave, etiqueta in (("formula_ancho", "Ancho"),
                                    ("formula_alto", "Alto"),
                                    ("formula_hojas", "Hojas")):
                ok, detalle = validar(datos.get(clave, ""), nombres)
                if not ok:
                    messagebox.showerror("Fórmula inválida", f"{etiqueta}: {detalle}")
                    return
            try:
                COMP.guardar_pano(self.db, datos, id_)
            except ValueError as exc:
                messagebox.showerror("No se puede armar así la composición", str(exc))
                return
            self.app.recargar_catalogos()

        self.abm_composicion = PanelABM(
            hoja, "Paños que forman la tipología (composición)",
            [("orden", "#", 45, "center"),
             ("etiqueta", "Paño", 160, "w"),
             ("tipologia_hijo", "Tipología", 90, "w"),
             ("formula_ancho", "Ancho", 100, "w"),
             ("formula_alto", "Alto", 150, "w"),
             ("linea_txt", "Línea", 100, "w")],
            cargar=cargar, campos=campos, al_guardar=guardar,
            al_borrar=lambda i: (self.db.borrar("tipologia_composicion", i),
                                 self.app.recargar_catalogos()),
            alto=7,
        )
        self.abm_composicion.grid(row=1, column=1, sticky="nsew", pady=(5, 6))
    ...
    def _panel_formulas(self, hoja):
        ...
        # Los combos arrancan con la primera opción puesta. Vacíos, la pestaña se
        # abre con la tabla en blanco y "Probar despiece" contesta "faltan datos"
        # sin que se entienda qué falta.
        ...
        self.combo_f_tip = CampoCombo(
            barra, "Tipología", tipologias, valor=tipologias[0] if tipologias else "",
        ...
        self.combo_f_linea = CampoCombo(
            barra, "Línea", ["(Genérica — todas las líneas)"] + lineas,
            valor=lineas[0] if lineas else "(Genérica — todas las líneas)",
            ancho=230, al_cambiar=lambda: self.abm_formulas.refrescar())
        self.combo_f_linea.pack(side="left")

        self.lbl_ayuda_formulas = ctk.CTkLabel(
            hoja, text=AYUDA_VARIABLES, font=tema.fuente(10),
            text_color=tema.c("texto_tenue"), justify="left", anchor="w")
        self.lbl_ayuda_formulas.grid(row=1, column=0, sticky="w", pady=(8, 0))
        ...
        def cargar():
            ...
            self._actualizar_ayuda_formulas(tip)
            if not tip:
                return []
            if es_generica():
                return self.db.query(
            ...
            return [
                ...
                {"key": "funcion", "label": "Función", "tipo": "combo",
                 ...
                 "ayuda": "HOJA_HORIZONTAL define AH y HOJA_VERTICAL define HH, "
                          "que usan las fórmulas de vidrio. Con piezas PREMARCO o "
                          "MOSQUITERO, esa casilla ya no se cobra entera por m²"},
        ...
        def guardar(id_, datos):
            ...
            if not tip:
                return
            # Las variables de la tipología son nombres válidos dentro de la
            # fórmula: sin pasarlas, si(PREMARCO, ...) se rechazaría.
            lid = None if es_generica() else self._id_linea(self.combo_f_linea.get())
            nombres = V.nombres(self.db, tip, lid)
            for clave in ("formula_largo", "cantidad_piezas"):
                ok, detalle = validar(datos[clave], nombres)
                if not ok:
    ...
    def _actualizar_ayuda_formulas(self, tipologia: str) -> None:
        """Suma a la ayuda las variables que declara esta tipología.

        Si no se listan, el usuario no tiene forma de saber que puede escribir
        si(PREMARCO, ...) — la función existe pero el nombre es invisible.
        """
        etiqueta = getattr(self, "lbl_ayuda_formulas", None)
        if etiqueta is None:
            return
        lid = None
        if not self.combo_f_linea.get().startswith("("):
            lid = self._id_linea(self.combo_f_linea.get())
        extra = V.ayuda(self.db, tipologia, lid) if tipologia else ""
        etiqueta.configure(text=AYUDA_VARIABLES + (f"\n{extra}" if extra else ""))
    ...
    def _formula_vidrio(self):
        ...
        if dialogo.resultado is None:
            return
        # Igual que en las fórmulas de despiece: las variables de la tipología
        # también son nombres válidos acá.
        nombres = V.nombres(self.db, tip, lid)
        for clave, valor in dialogo.resultado.items():
            ok, detalle = validar(valor, nombres)
            if not ok:
                messagebox.showerror("Fórmula inválida", f"{clave}: {detalle}")
                return
    ...
    def _probar_despiece(self):
        ...
        despiece = calcular_despiece(
            ...
            # Con las opciones de la tipología en su valor por defecto: sin ellas,
            # una fórmula como si(PREMARCO, H+36, 0) daría "Variable desconocida".
            variables=V.valores_por_defecto(self.db, tip, lid))
