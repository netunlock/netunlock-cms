...
PARAMETROS = [
    ...
    # Cuando la línea corta el mosquitero con sus perfiles (corredizas MDT), el
    # aluminio va por peso y por m² se cobra sólo la tela. Arranca en $ 0.
    ("mosquitero_tela_m2", 0, "Tela de mosquitero por m² ($), si la línea corta sus perfiles",
     "Opcionales"),
...
def sembrar(db) -> None:
    """Carga lo mínimo que toda instalación necesita: los datos de la empresa
    y los parámetros. Se ejecuta una sola vez, al crear la base.

    Ni líneas, ni perfiles, ni vidrios, ni accesorios: todo eso lo trae
    :func:`core.catalogo_mdt.sembrar_mdt` con los dos catálogos del fabricante
    y **en $0**, para que cada uno cargue su lista de precios.

    Lo que había antes —Módena, Herrero, A30 New, Ekonal, y sus vidrios y
    accesorios con precios inventados— quedó en :func:`sembrar_muestra`, que
    sólo usa la regresión. En un programa que se baja de internet, un precio
    inventado es peor que ningún precio: quien lo abre no sabe si ese número
    es de verdad.
    """
...
def sembrar_muestra(db) -> None:
    """Carga el juego de líneas de ejemplo. **Sólo para la regresión.**

    ``tools.prueba_rapida`` compara contra totales fijos —un despiece de 1500 x
    1100 tiene que dar exactamente estos milímetros, este presupuesto tiene que
    dar exactamente este total—. Para eso necesita datos que no cambien y
    precios que no cambien. Los catálogos MDT no sirven para eso: son reales,
    pero vienen en $0 a propósito, así que no habría número contra el cual
    verificar.

    No se llama nunca desde la aplicación. Si estos datos aparecen en la base de
    un cliente, algo se llamó de más.
    """
    ids_linea: dict[str, int] = {}

    # --- Vidrios
    # Se saltean los que ya estén: los catálogos MDT traen algunos con el mismo
    # nombre ("DVH 4/9/4"), y esta función corre DESPUÉS de ellos.
    ya_hay = {v["nombre"] for v in db.query("SELECT nombre FROM vidrios")}
    ...
    # --- Accesorios
    ids_acc = {f["codigo"]: f["id"]
               for f in db.query("SELECT id, codigo FROM accesorios")}
    for codigo, desc, unidad, precio in ACCESORIOS:
        if codigo in ids_acc:
            continue
    ...
    # --- Tipologías
    # Las tipologías de ejemplo van con las líneas de ejemplo: sus fórmulas son
    # genéricas y producen el "despiece genérico", que da números plausibles y
    # equivocados. Las 30 de MDT que carga catalogo_mdt traen las medidas del
    # fabricante, así que no hacen falta.
